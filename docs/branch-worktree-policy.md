...
## 禁止
- `main` / `develop` 直下で実装しない（足場・Issue 整備のような例外は明示する）
...
# Issue に紐づく開発ブランチを作成
gh issue develop 12 --name issue/12-pytest-foundation --base main
...
git worktree add -b issue/12-pytest-foundation \
  ../worktrees/synthetic-research-data-sharing-issue-12 main
...
gh pr create --base main --title "<要約> (#12)" --body-file pr-body.md   # host 一致時
