...
- `register` ビュー（owner 専用）に **「デモ用データを登録」ボタン**を備える。手入力なしで
  **同梱の架空デモデータセット**を 1 件登録し、登録直後に新規データセットの個別ページを開く
  （「登録 → カタログ反映 → 個別ページ」を素早く見せるための便宜機能）。
- 手入力フォームと**同一の `catalog.register()` 経路**を通すため、登録結果は手動登録と同形・同挙動。
  デモデータ定義はページ内 JS 定数（複数件をクリックごとに巡回）。**アーキテクチャは不変**で、
  追加のガード・ストア・on-disk ファイルは作らない（owner 専用 `register` ビュー内に閉じる）。
- デモデータも `user-<n>` 採番のユーザー登録分であり、`data/<id>/synthetic.json` を**持たない**ため、
