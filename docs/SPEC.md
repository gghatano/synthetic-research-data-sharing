...
> **位置づけ（#43, N4）**: この「合成 vs 生の核メッセージ」は、アプリで運用する実コンテンツ
> （カタログ閲覧・提出・審査）ではなく**デモ成果物**である。そのため**データカタログには置かず**、
> カタログ外の専用デモ面（§7.10）に配置する。カタログ詳細・提出・審査の各ビューには合成 vs 生の
> 並置を出さない。
...
- **分析プログラム＋結果の提出（#41, N2）**: ブラウザ内の「分析ワークベンチ」（3 分析を
  ...
  - **「合成 vs 生（合成≈生）の核メッセージ」はカタログ個別ページには置かない**（デモ成果物
    として §7.10 のカタログ外デモ面へ移設、#43, N4）。
...
> **改訂（#42, N3）**: #41 で提出が「プログラム＋結果アップロード」型へ変わったため、旧審査
> （提出物一覧から合成結果フラグメントを読み、承認で生データ結果を並置）は撤去した。**生データ
> 並置（合成 vs 生）の核メッセージはカタログ／審査には置かず**、デモ成果物としてカタログ外の
> 専用デモ面（§7.10, #43）へ移設する。
...
| 合成≈生の核メッセージ | カタログ外の専用デモ面（§7.10） | 解説／オンボーディング面 |

## 7.10 原理デモ面（カタログ外, #43）

「同一コードが合成・生で動き、結果は近いが非同一」という**核メッセージ**（§1）は、アプリの
実コンテンツではなく**デモ成果物**である。データカタログ（一覧・個別ページ）・提出・審査の
どのビューにも置かず、**カタログ外の専用デモ面**（`demo` ビュー）に配置する（#39 確定方針）。

### 到達と配置

- アプリバー下の主ナビ「原理デモ」（`nav-demo`）とフッタのリンク（`foot-demo-link`）から到達する。
  審査ビューの承認 callout（`review-demo-link`）からも誘導する。**誰でも閲覧可**（`demo` は公開
  ビュー = `isPublicView`）。
- ビューは `site/index.html` の Alpine ストア `session.view === 'demo'`（`demoPage()`）。

### 表示（合成 vs 生の並置）

- 1 データセット（`prostate-psa`）で、3 分析（`clustering` / `association` / `survival`）から
  選んで、**合成データの結果**（`fragments/<id>/analyst/<a>.html`）と**生データの結果**
  （`fragments/<id>/owner/<a>.html` = TRE 内で事前計算）を**左右に並置**する（`htmx.ajax` GET）。
- 並置後、**「合成 → 生」が近いが非同一**である核メッセージ（`demo-callout`）を表示する。
  「これはデモ成果物であり、生データは TRE に隔離され公開しない（事前生成結果のみ読込）」旨を
  注記する（`demo-disclaimer`）。
- canvas id は合成 `chart-synthetic-<a>` / 生 `chart-raw-<a>` で**別 id**のため並置しても衝突
  しない。分析切替・再選択での二重描画はフラグメント側の `Chart.getChart(el).destroy()` で防ぐ。

### 旧 3 ロールデモとの関係

- 旧 3 ロールデモ（ワークベンチの公開/分析/審査タブ）は #41 で撤去済み。核メッセージはこの
  デモ面に一本化した。
...
| 関心事 | モック（本リポジトリ） | 本番プロトタイプ |
|--------|------------------------|------------------|
| 合成 vs 生の並置 | 事前生成フラグメント（analyst/owner）を `htmx.ajax` で読込 | 解説面で同一ジョブの 2 結果を提示 |
| 生データ隔離 | 「非公開」表示＋事前生成 raw フラグメントのみ | TRE に隔離、生個票へは直接触れない |
| 位置づけ | カタログ外の独立ビュー（デモ成果物） | オンボーディング／解説コンテンツ |
