...
> **改訂予定（#42, N3）**: #41 で提出が「プログラム＋結果アップロード」型へ変わり、旧審査
> （提出物一覧の各カードから合成結果フラグメントを読み、承認で生データ結果を並置）は**撤去**
> 済み。本節は #42 で「提出物（プログラム＋結果＋レポート）の閲覧 → 承認」へ全面改訂する。
> 「合成 vs 生（合成≈生）の核メッセージ」はカタログ外のデモ面（§7.10, #43）へ移設する。
> 以下は #26 当時の記述（履歴）。
